...
main :: IO ()
main = do
  S.writeFile fp $ contents 10000 -- 10000 cannot be handled fileParser0 and 1
  ...
  where
    parse_file parser = IO.withBinaryFile fp IO.ReadMode $ \h ->
      do
        p' <- runEffect $ parseWith parser (Bytes.fromHandle h) >-> PP.print
        runEffect $ p' >-> PP.print
    parseWith parser = loop
      where
        loop p = do
          (m, p') <- lift (runStateT (runMaybeT parser) p)
          case m of
            Nothing -> return p'
            Just file -> do
              yield file
...
fileParser1 = do
  nameLength <- zoom utf8 parseNumber
...
fileParser2 = do
  nameLength <- zoom (Bytes.splitAt 3 . utf8) parseNumber
